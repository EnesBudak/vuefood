...
const state = {
  products:[],
  userInfos:[{"username":"33","firstname":"Enes","lastname":"Budak","email":"enes@hotmail.com","adress":"Bey mahallesi 16030 sokak no 15 kat 2","adress_2":"Barbaros mahallesi 16030 sokak no 1 kat 7","phone":"5435292117","product":[],"cardTotal":0,"orderCount":0}],
  cartItems:[],
  orderInfos:[],
  userOrderInfos:[],
...
const actions = {
 async fetchProducts({commit}){
  ...
  addCartItem({commit},payload){
    ...
    axios.post('user/sepet',{id :payload.id,count: payload.count,options:payload.options})
    .then( (response) =>{
...
const mutations = {
  ...
  decrement(state,payload){
    if(state.products[payload.index1].menuItems[payload.index2].quantity == 1){
